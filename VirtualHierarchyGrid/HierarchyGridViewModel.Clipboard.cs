...
namespace VirtualHierarchyGrid
{
    partial class HierarchyGridViewModel
    {
        private DataObject CopyToClipboard(string title, bool withStructure, IEnumerable<HierarchyDefinition> rows, IEnumerable<HierarchyDefinition> columns)
        {
            ...
            WriteXMLColumnsHeaders(writer, maxLevel, columns);

            // Rows...
            var colLeaves = ColumnsDefinitions.Leaves().ToList();

            foreach (HierarchyDefinition rowDef in rows)
            {
                ...
                WriteXMLData(writer, columns ?? colLeaves, rowDef);
        ...
        private void WriteXMLData(XmlTextWriter writer, IEnumerable<HierarchyDefinition> colLeaves, HierarchyDefinition rowDef)
        {
            foreach (var colDef in colLeaves)
            {
                ...
                if (idd.IsNone)
                    continue; // Skip loop if we don't have a valid producer/consumer couple
        ...
        private void WriteXMLColumnsHeaders(XmlTextWriter writer, int maxLevel, IEnumerable<HierarchyDefinition> consumers)
        {
            if (consumers == null)
            {
                for (int level = 0; level < consumers.Select(o => o.Depth(true)).Max(); level++)
                {
                    writer.WriteStartElement("Row");
                    ...
                    //int max = ColumnsLevel.Keys.Max();
                    var flat = consumers.FlatList().ToArray();

                    foreach (HierarchyDefinition colDef in flat.Where(x => x.Level == level))
                    {
                        writer.WriteStartElement("Cell");
            ...
            else
            {
                ...
                foreach (HierarchyDefinition colDef in consumers)
                {
                    writer.WriteStartElement("Cell");
