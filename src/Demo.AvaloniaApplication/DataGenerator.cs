    ...
    public class DataGenerator
    {
        ...
        static DataGenerator()
        {
            _regions = new Dictionary<string, string[]>
            {
                {
                    "Europe",
                    new[] { "Benelux", "France", "Germany", "United Kingdom", "Italy", "Spain" }
                },
        ...
        private int GetData(string region, string vehicle)
        {
            ...
            if (
                regions.Length == 1
                && vehicles.Length == 1
                && regions[0].Equals(region)
                && vehicles[0].Equals(vehicle)
            )
                return 0;
        ...
        public HierarchyDefinitions GenerateSample() => new(BuildProducers(), BuildConsumers());

        private IEnumerable<ProducerDefinition> BuildProducers() =>
            new[]
            {
        ...
        {
            var prd = new ProducerDefinition { Content = region, Tooltip = region };
        ...
        {
            var csr = new ConsumerDefinition { Content = vehicle, Tooltip = vehicle };
            ...
            else
            {
                ...
                {
                    if (
                        Data.TryGetValue(((string)data, vehicle), out var _)
