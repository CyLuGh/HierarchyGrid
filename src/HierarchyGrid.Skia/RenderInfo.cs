...
namespace HierarchyGrid.Skia
{
    internal struct RenderInfo
    {
        ...
        private static SKColor FindForegroundColor(
            HierarchyGridViewModel viewModel,
            SkiaTheme theme,
            PositionedCell cell
        )
        ...
        internal static RenderInfo FindRender(
            HierarchyGridViewModel viewModel,
            SkiaTheme theme,
            PositionedCell cell
        ) =>
        ...
        private static SKColor FindBackgroundColor(
            ...
            SkiaTheme theme,
            HierarchyDefinition hdef,
            Option<PositionedCell> hoveredCell
        ) =>
            hoveredCell.Match(
                cell =>
                {
                    if (
                        (
                            hdef is ConsumerDefinition consumer
                            && cell.ConsumerDefinition.Equals(consumer)
                        )
                        || (
                            hdef is ProducerDefinition producer
                            && cell.ProducerDefinition.Equals(producer)
                        )
                    )
                    {
        ...
        {
            if (hdef?.DefinitionId.Equals(viewModel.HoveredElementId) == true)
                return theme.HoverHeaderBackgroundColor;
            ...
            return IsHovered(viewModel, hdef)
