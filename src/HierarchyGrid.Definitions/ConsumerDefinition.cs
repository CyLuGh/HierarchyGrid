    ...
    public class ConsumerDefinition : HierarchyDefinition
    {
        ...
        public ResultSet Process(InputSet inputSet)
        {
            ...
            if (ContextItems != null)
            {
                ...
                if (cis.Length > 0)
                {
                    resultSet.ContextCommands = Option<(string, ICommand)[]>.Some(
                        cis.Select(ci =>
                                (
                                    ci.description,
                                    (ICommand)ReactiveCommand.Create(() => ci.action(resultSet))
                                )
                            )
                            .ToArray()
                    );
            ...
            var tooltipText =
                TooltipCreator != null ? TooltipCreator(inputSet.Input, data) : string.Empty;
