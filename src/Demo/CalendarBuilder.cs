...
namespace Demo
{
    public class CalendarBuilder
    {
        ...
        public IEnumerable<ProducerDefinition> GetProducers() =>
            Enumerable
                .Range(DateTime.Today.Year - 1, 3)
                .Select(year =>
                {
                    var yearlyProducer = new ProducerDefinition(
                        _producersId.GetOrCreate($"{year}", () => Guid.NewGuid())
                    )
                    {
                        Content = $"{year}",
                    };
                    ...
                    Enumerable
                        ...
                        .ForEach(month =>
                        {
                            var monthlyProducer = yearlyProducer.Add(
                                new ProducerDefinition(
                                    _producersId.GetOrCreate(
                                        $"{year}.{month}",
                                        () => Guid.NewGuid()
                                    )
                                )
                            ...
                            _users
                                ...
                                .ForEach(user =>
                                {
                                    var userProducer = monthlyProducer.Add(
                                        new ProducerDefinition(
                                            _producersId.GetOrCreate(
                                                $"{year}.{month}.{user}",
                                                () => Guid.NewGuid()
            ...
            Enumerable
                ...
                .Select(day =>
                {
                    var consumer = new ConsumerDefinition(
                        _consumersId.GetOrCreate(day, () => Guid.NewGuid())
                    )
                    {
                        Content = $"{day}",
                        Consumer = o =>
